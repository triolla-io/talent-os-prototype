...
const TYPE_CONFIG: Record<
  AIInsight["type"],
  { icon: typeof Lightbulb; iconClass: string; bgClass: string }
> = {
  prediction: {
    icon: Lightbulb,
...
export function AIInsightsPanel() {
  return (
    <motion.div
      ...
      className="ai-glow-border rounded-xl h-full"
    >
      <div className="bg-gradient-to-br from-ai-amber/[0.03] to-ai-coral/[0.03] rounded-xl p-5 h-full flex flex-col">
        {/* Header */}
        <div className="flex items-center gap-2 mb-4">
          <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-ai-amber to-ai-coral flex items-center justify-center shrink-0">
                    ...
                    <div className="mt-1.5 flex items-center gap-1.5">
                      <div className="h-1 flex-1 rounded-full bg-muted overflow-hidden">
                        <motion.div
                          ...
                          className="h-full rounded-full bg-gradient-to-r from-ai-amber to-ai-coral"
                        />
                      </div>
                      <span className="text-[10px] font-semibold text-ai-amber shrink-0">
                        {insight.confidence}%
                      </span>
                    </div>
                  </div>
                </div>
                {i < mockInsights.length - 1 && (
